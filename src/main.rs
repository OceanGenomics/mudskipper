...
fn main() {
    ...
    let trees = annotations::build_tree(&ann_file_adr, &mut transcripts_map, &mut transcripts, &mut tx_lengths).expect("cannot build the tree!");
    annotations::test_tree(&ann_file_adr, &trees);
    let bam_file_adr: String = env::args().nth(2).expect("missing src");
    bam::read_bamfile(&bam_file_adr, &transcripts, &tx_lengths, &trees);
}